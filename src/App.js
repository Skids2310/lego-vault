...
const RECORD_ID = 1;
const ADMIN_PASSWORD = "legovault2024";
...
async function dbSave(data) {
  await fetch(`${SUPABASE_URL}/rest/v1/sets`, {
    ...
    body: JSON.stringify({ id: RECORD_ID, data: JSON.stringify(data) })
  });
}
...
function getImageUrls(setNumber) {
  ...
  const w = n.includes("-") ? n : n + "-1";
  return [
    `https://images.brickset.com/sets/images/${w}.jpg`,
    `https://images.brickset.com/sets/small/${w}.jpg`,
    `https://cdn.rebrickable.com/media/sets/${w}.jpg`,
  ];
}
...
function exportCSV(data, filename="lego-collection") {
  ...
  const rows = data.map(s => {
    ...
    return [s.name, s.setNumber||"", s.theme||"", s.year||"", s.pieces||"", s.minifigs||"", s.status||"", s.folder||"",
      paid>0?paid.toFixed(2):"", val>0?val.toFixed(2):"", gain, s.rating||"", (s.notes||"").replace(/,/g," "),
...
const THEME_COLORS = { "Harry Potter":"#7b2d8b","City":"#0d6efd","Creator Expert":"#c1121f","Icons":"#c1121f","Creator":"#fd7e14","Classic":"#ffc107","Botanicals":"#2d8b4e","Architecture":"#6c757d","Ideas":"#20c997","Technic":"#e63946","Star Wars":"#212529","Marvel":"#e63946","Speed Champions":"#fd7e14","Seasonal":"#20c997","Sports":"#0d6efd","Other":"#6c757d","Ikea":"#0057a8","Monkie Kid":"#e63946","The Lego Movie 2":"#ffc107","Xtra":"#6c757d" };
...
const LEGO_BRICK_SVG = `<svg xmlns='http://www.w3.org/2000/svg' width='60' height='50' viewBox='0 0 60 50'><rect x='2' y='14' width='56' height='34' rx='3' fill='none' stroke='%23ffffff' stroke-width='1.5' opacity='0.15'/><rect x='8' y='8' width='14' height='10' rx='3' fill='none' stroke='%23ffffff' stroke-width='1.5' opacity='0.15'/><rect x='24' y='8' width='14' height='10' rx='3' fill='none' stroke='%23ffffff' stroke-width='1.5' opacity='0.15'/><rect x='40' y='8' width='14' height='10' rx='3' fill='none' stroke='%23ffffff' stroke-width='1.5' opacity='0.15'/></svg>`;
...
function SetImage({ setNumber, imageUrl, size="card" }) {
  ...
  const h = size==="card" ? "160px" : "240px";
  useEffect(()=>{ setSrcIndex(0); setFailed(false); }, [setNumber, imageUrl]);
  function onError() { if (srcIndex+1<urls.length) setSrcIndex(i=>i+1); else setFailed(true); }
  if (!urls.length||failed) return (
    <div style={{ height:h, background:"#0a0a14", borderRadius:size==="card"?"8px 8px 0 0":"8px", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center" }}>
      ...
      <div style={{ fontSize:"0.6rem", color:"#333", fontFamily:"sans-serif", marginTop:"0.3rem" }}>{setNumber?"No image":"No set #"}</div>
    </div>
  );
  return (
...
export default function LegoDatabase() {
  ...
  async function loadSets() {
    try {
      ...
      if (remote) { setSets(remote); localStorage.setItem("lego-sets-v2", JSON.stringify(remote)); }
      else {
        const local = localStorage.getItem("lego-sets-v2");
        const data = local ? JSON.parse(local) : SEED_DATA;
        setSets(data); await dbSave(data);
      }
    } catch(e) {
      const local = localStorage.getItem("lego-sets-v2");
  ...
  async function saveSets(u) {
    try { localStorage.setItem("lego-sets-v2", JSON.stringify(u)); await dbSave(u); }
  ...
  function handleEdit(set) { setForm({...set}); setEditId(set.id); setShowForm(true); setExpandedSet(null); setSelectedSet(null); window.scrollTo({top:0,behavior:"smooth"}); }
  ...
  function markOwned(id) { const u=sets.map(s=>s.id===id?{...s,status:"Built"}:s); setSets(u); saveSets(u); showToast("🧱 Marked as owned!"); }

  function attemptLogin() {
    if (passwordInput===ADMIN_PASSWORD) { localStorage.setItem("lego-admin",ADMIN_PASSWORD); setIsAdmin(true); setShowPasswordPrompt(false); setPasswordInput(""); setPasswordError(false); showToast("🔓 Admin unlocked!"); }
    else { setPasswordError(true); setPasswordInput(""); }
  }
  function logout() { localStorage.removeItem("lego-admin"); setIsAdmin(false); showToast("🔒 Logged out"); }

  function toggleBulkSelect(id) { setSelectedSets(prev=>{ const n=new Set(prev); n.has(id)?n.delete(id):n.add(id); return n; }); }
  function applyBulkEdit() {
    if (!selectedSets.size) return;
    const u = sets.map(s => !selectedSets.has(s.id)?s:{...s,...(bulkStatus?{status:bulkStatus}:{}),...(bulkFolder?{folder:bulkFolder}:{})});
    setSets(u); saveSets(u); setSelectedSets(new Set()); setBulkMode(false); setBulkStatus(""); setBulkFolder(""); showToast(`✅ Updated ${selectedSets.size} sets`);
  }

  async function bulkFetchPrices() {
    const toFetch = sets.filter(s=>s.setNumber&&!s.currentValue);
    if (!toFetch.length) return showToast("All sets already have prices!");
    setFetchingPrices(true); setFetchProgress(0);
    let updated = [...sets]; let count = 0;
    for (const set of toFetch) {
      try {
        const bs = await fetchBricksetSet(set.setNumber);
        if (bs) { const price = bs.LERetailPrice||bs.AURetailPrice||bs.USRetailPrice||""; if (price) { updated=updated.map(s=>s.id===set.id?{...s,currentValue:String(price)}:s); count++; } }
        await new Promise(r=>setTimeout(r,200));
      } catch(e) {}
      setFetchProgress(Math.round(((toFetch.indexOf(set)+1)/toFetch.length)*100));
    }
    setSets(updated); saveSets(updated); setFetchingPrices(false); setFetchProgress(0); showToast(`✅ Fetched prices for ${count} sets!`);
  }
  ...
  async function lookupSetNumber(setNumber) {
    ...
    try {
      ...
      setForm(f=>({...f, name:f.name||s.name||f.name, year:f.year||String(s.year||""), pieces:f.pieces||String(s.pieces||""), theme:THEMES.includes(s.theme)?s.theme:f.theme, minifigs:f.minifigs||String(s.minifigs||""), currentValue:f.currentValue||(price?String(price):"") }));
      showToast("✅ Details loaded from Brickset!");
    } catch(e) {}
  }
  ...
  async function importFromUrl(status="Wanted") {
    ...
    if (!match) return showToast("Couldn't find a set number in that URL","error");
    ...
    setUrlLoading(true); showToast("🔍 Looking up set details...");
    try {
      let bsData = null;
      ...
      try {
        const response = await fetch("https://api.anthropic.com/v1/messages", {
          ...
          body: JSON.stringify({ model:"claude-sonnet-4-20250514", max_tokens:300, messages:[{ role:"user", content:`LEGO set ${setNumber} (${guessedName}). Reply ONLY with JSON, no markdown: {"name":"full set name","theme":"theme","year":"4 digit year","pieces":"number","minifigs":"number","minifigNames":"comma separated names or empty"}` }] })
        });
      ...
      const newSet = { ...EMPTY_FORM, id:Date.now().toString(), addedAt:Date.now(), status, name:bsData?.name||info.name||guessedName, setNumber, theme:bsData?.theme||info.theme||"Other", year:bsData?.year?String(bsData.year):(info.year||""), pieces:bsData?.pieces?String(bsData.pieces):(info.pieces||""), minifigs:bsData?.minifigs?String(bsData.minifigs):(info.minifigs||""), currentValue:price?String(price):"", notes };
      const u=[newSet,...sets]; setSets(u); saveSets(u); setUrlInput("");
      showToast(`${status==="Wanted"?"⭐":"🧱"} Added "${newSet.name}"!`);
    } catch(e) {
      const newSet={...EMPTY_FORM,id:Date.now().toString(),addedAt:Date.now(),status,name:guessedName,setNumber};
      const u=[newSet,...sets]; setSets(u); saveSets(u); setUrlInput(""); showToast(`Added "${guessedName}" — edit to add more details`);
    }
    setUrlLoading(false);
  }
  ...
  sets.forEach(set => {
    if (set.notes&&set.notes.startsWith("Minifigs: ")) {
      ...
      names.forEach(name => { if (name&&name!=="Various"&&name!=="0") allMinifigs.push({name, setName:set.name, setNumber:set.setNumber, theme:set.theme, imageUrl:set.imageUrl}); });
    }
  });
  const filteredMinifigs = minifigSearch ? allMinifigs.filter(m=>m.name.toLowerCase().includes(minifigSearch.toLowerCase())||m.setName.toLowerCase().includes(minifigSearch.toLowerCase())) : allMinifigs;
  ...
  const filtered = owned.filter(s => {
    ...
    switch(sortBy) {
      ...
      case "gain_desc": { const ga=(parseFloat(a.currentValue)||0)-(parseFloat(a.purchasePrice)||0); const gb=(parseFloat(b.currentValue)||0)-(parseFloat(b.purchasePrice)||0); return gb-ga; }
  ...
  return (
    <div style={{ fontFamily:"'Georgia',serif", minHeight:"100vh", color:"#fffffe", background:"#0f0e17", backgroundImage:BRICK_BG, backgroundSize:"70px 58px" }}>
      ...
      <div style={{ position:"relative", zIndex:1 }}>
        ...
        <div style={{ background:"linear-gradient(135deg,#e63946 0%,#c1121f 50%,#780000 100%)", padding:"2rem", textAlign:"center", borderBottom:"4px solid #ffd60a", position:"relative", overflow:"hidden" }}>
          ...
          <div style={{ position:"relative" }}>
            ...
            <div style={{ marginTop:"0.5rem" }}>
              {isAdmin ? <button onClick={logout} style={{ background:"transparent",color:"#ffffff88",border:"1px solid #ffffff33",padding:"0.25rem 0.75rem",borderRadius:"100px",fontSize:"0.7rem",cursor:"pointer",fontFamily:"sans-serif" }}>🔓 Admin · Lock</button>
            ...
            <div style={{ display:"flex", gap:"1.5rem", justifyContent:"center", marginTop:"1rem", flexWrap:"wrap" }}>
              {[["Sets Owned",owned.length],["Total Pieces",totalPieces.toLocaleString()],["Wishlist",wishlist.length],["Minifigs",allMinifigs.length],
        ...
        {/* Tabs */}
        <div style={{ display:"flex", borderBottom:"2px solid #2a2a4a", background:"rgba(15,14,23,0.9)" }}>
          {TABS.map(tab=>(
            <button key={tab} onClick={()=>{setActiveTab(tab);setShowForm(false);}}
              style={{ flex:1, padding:"0.85rem", background:"none", border:"none", borderBottom:activeTab===tab?"3px solid #ffd60a":"3px solid transparent", color:activeTab===tab?"#ffd60a":"#666", fontFamily:"'Impact',sans-serif", letterSpacing:"0.1em", fontSize:"0.95rem", cursor:"pointer" }}>
        ...
        <div style={{ maxWidth:"1100px", margin:"0 auto", padding:"1.5rem 1rem" }}>
            ...
            <>
              ...
              {!showForm&&isAdmin&&(
                <div style={{ background:"rgba(26,26,46,0.92)",border:"1px solid #e6394633",borderRadius:"12px",padding:"1.25rem",marginBottom:"1.5rem",backdropFilter:"blur(4px)" }}>
                  <div style={{ fontSize:"0.8rem",color:"#e63946",fontFamily:"sans-serif",fontWeight:"700",marginBottom:"0.4rem",letterSpacing:"0.05em",textTransform:"uppercase" }}>⚡ Quick Add from LEGO.com URL</div>
                  <div style={{ fontSize:"0.72rem",color:"#666",fontFamily:"sans-serif",marginBottom:"0.75rem" }}>Paste any lego.com product URL and we'll look up all details automatically</div>
                  <div style={{ display:"flex",gap:"0.5rem" }}>
                    <input value={urlInput} onChange={e=>setUrlInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&importFromUrl("Built")} placeholder="https://www.lego.com/en-au/product/..." style={{...IS,flex:1,background:"rgba(15,14,23,0.8)"}}/>
                    <button onClick={()=>importFromUrl("Built")} disabled={urlLoading} style={{ background:urlLoading?"#333":"#e63946",color:"#fff",border:"none",padding:"0.55rem 1.25rem",borderRadius:"6px",fontWeight:"900",cursor:urlLoading?"not-allowed":"pointer",fontFamily:"'Impact',sans-serif",fontSize:"0.9rem",whiteSpace:"nowrap" }}>{urlLoading?"...":"ADD 🧱"}</button>
                  </div>
                </div>
              )}
              ...
              {showForm&&(
                <div style={{ background:"rgba(26,26,46,0.95)",border:"2px solid #e63946",borderRadius:"12px",padding:"1.5rem",marginBottom:"2rem",backdropFilter:"blur(4px)" }}>
                  ...
                  {(form.setNumber||form.imageUrl)&&<div style={{ marginBottom:"1.25rem",maxWidth:"260px" }}><SetImage setNumber={form.setNumber} imageUrl={form.imageUrl} size="card"/></div>}
                  <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(190px,1fr))",gap:"1rem" }}>
                    {[["Set Name *","name","text","e.g. Eiffel Tower"],["Set Number","setNumber","text","e.g. 10307"],["Year","year","number","e.g. 2023"],["Piece Count","pieces","number","e.g. 10001"],["Purchase Price ($)","purchasePrice","number","e.g. 49.99"],["RRP / Current Value ($)","currentValue","number","e.g. 89.99"]].map(([label,field,type,ph])=>(
                      <label key={field} style={LS}>{label}
                        <input type={type} placeholder={ph} value={form[field]||""} onChange={e=>setForm(f=>({...f,[field]:e.target.value}))} onBlur={field==="setNumber"?()=>lookupSetNumber(form.setNumber):undefined} style={IS}/>
                      </label>
                    ))}
                    ...
                    <label style={LS}>Folder<input type="text" placeholder="e.g. Aqua, Red" value={form.folder||""} onChange={e=>setForm(f=>({...f,folder:e.target.value}))} style={IS}/></label>
                  ...
                  <div style={{ marginTop:"0.5rem",fontSize:"0.7rem",color:"#555",fontFamily:"sans-serif" }}>💡 Tab out of Set Number to auto-fill from Brickset</div>
              ...
              <div style={{ display:"flex",gap:"0.75rem",flexWrap:"wrap",marginBottom:"1rem",alignItems:"center" }}>
                ...
                <select value={sortBy} onChange={e=>setSortBy(e.target.value)} style={{...IS,background:"rgba(26,26,46,0.9)"}}>
                  <optgroup label="Date"><option value="added">Recently Added</option><option value="year_desc">Year (Newest)</option><option value="year_asc">Year (Oldest)</option></optgroup>
                  <optgroup label="Name"><option value="name_az">Name A–Z</option><option value="name_za">Name Z–A</option></optgroup>
                  <optgroup label="Size"><option value="pieces_desc">Most Pieces</option><option value="pieces_asc">Fewest Pieces</option><option value="minifigs_desc">Most Minifigs</option></optgroup>
                  <optgroup label="Price"><option value="value_desc">Highest RRP</option><option value="value_asc">Lowest RRP</option><option value="paid_desc">Highest Paid</option><option value="paid_asc">Lowest Paid</option><option value="gain_desc">Biggest Gain</option></optgroup>
                  <optgroup label="Other"><option value="rating_desc">Highest Rated</option><option value="theme">Theme A–Z</option><option value="folder">Folder A–Z</option></optgroup>
                </select>
              </div>
                  ...
                  {fetchingPrices?(
                    <div style={{ background:"rgba(26,26,46,0.9)",border:"1px solid #2a2a4a",borderRadius:"8px",padding:"0.75rem 1rem",display:"flex",alignItems:"center",gap:"1rem" }}>
                      <div style={{ flex:1 }}>
                        ...
                        <div style={{ background:"#0f0e17",borderRadius:"100px",height:"6px",overflow:"hidden" }}><div style={{ width:`${fetchProgress}%`,height:"100%",background:"#22c55e",borderRadius:"100px",transition:"width 0.3s" }}/></div>
                      </div>
                    </div>
                  ):sets.filter(s=>s.setNumber&&!s.currentValue).length>0&&(
                    <button onClick={bulkFetchPrices} style={{ background:"rgba(34,197,94,0.1)",color:"#22c55e",border:"1px solid #22c55e44",padding:"0.5rem 1rem",borderRadius:"8px",cursor:"pointer",fontSize:"0.78rem",fontFamily:"sans-serif",fontWeight:"700" }}>
              ...
              {loading?<div style={{ textAlign:"center",color:"#aaa",padding:"3rem",fontFamily:"sans-serif" }}>Loading vault...</div>:(
                    ...
                    <div key={set.id} onClick={()=>bulkMode?toggleBulkSelect(set.id):setExpandedSet(expandedSet===set.id?null:set.id)}
                      ...
                      <div style={{ padding:"0.85rem",display:"flex",flexDirection:"column",gap:"0.4rem" }}>
                        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:"0.5rem" }}>
                          {bulkMode&&<span style={{ fontSize:"1rem",flexShrink:0 }}>{selectedSets.has(set.id)?"✅":"⬜"}</span>}
                          <div style={{ fontSize:"0.9rem",fontWeight:"700",color:"#fffffe",lineHeight:1.3,flex:1 }}>{set.name}</div>
                          <span style={{ background:(STATUS_COLORS[set.status]||"#666")+"22",color:STATUS_COLORS[set.status]||"#666",border:`1px solid ${STATUS_COLORS[set.status]||"#666"}55`,fontSize:"0.58rem",fontFamily:"sans-serif",fontWeight:"700",padding:"0.12rem 0.4rem",borderRadius:"100px",whiteSpace:"nowrap",letterSpacing:"0.04em",textTransform:"uppercase",flexShrink:0 }}>{set.status}</span>
                        </div>
                        {set.setNumber&&<div style={{ fontSize:"0.7rem",color:"#ffd60a",fontFamily:"monospace" }}>#{set.setNumber}</div>}
                        <div style={{ display:"flex",gap:"0.4rem",flexWrap:"wrap",alignItems:"center" }}>
                          <span style={{ width:"8px",height:"8px",borderRadius:"50%",background:THEME_COLORS[set.theme]||"#666",flexShrink:0,display:"inline-block" }}/>
                          <Chip icon="" val={set.theme}/>
                        ...
                        {(set.purchasePrice||set.currentValue)&&(
                          <div style={{ display:"flex",gap:"0.6rem",fontFamily:"sans-serif",fontSize:"0.72rem",marginTop:"0.1rem",flexWrap:"wrap",alignItems:"center" }}>
                            ...
                            {set.purchasePrice&&set.currentValue&&(()=>{ const g=parseFloat(set.currentValue)-parseFloat(set.purchasePrice); return <span style={{ color:g>=0?"#22c55e":"#ef4444",fontWeight:"700" }}>{g>=0?"▲":"▼"}${Math.abs(g).toFixed(2)}</span>; })()}
                          </div>
                        )}
                        ...
                        {expandedSet===set.id&&(
                          <div style={{ marginTop:"0.4rem",paddingTop:"0.75rem",borderTop:"1px solid #2a2a4a" }}>
                            ...
                            <div style={{ display:"flex",gap:"0.5rem",flexWrap:"wrap" }} onClick={e=>e.stopPropagation()}>
                              ...
                              {set.setNumber&&<a href={legoUrl(set.setNumber,set.name)} target="_blank" rel="noreferrer" onClick={e=>e.stopPropagation()} style={{ flex:1,background:"#ffd60a22",color:"#ffd60a",border:"1px solid #ffd60a44",padding:"0.4rem",borderRadius:"6px",cursor:"pointer",fontSize:"0.75rem",fontFamily:"sans-serif",textDecoration:"none",textAlign:"center",display:"block" }}>🛒 LEGO.com</a>}
                            </div>
                          </div>
                        )}
                      </div>
          ...
          {activeTab==="Wishlist"&&(
            <>
              ...
              {isAdmin&&<div style={{ background:"rgba(26,26,46,0.92)",border:"1px solid #ffd60a33",borderRadius:"12px",padding:"1.25rem",marginBottom:"1.5rem",backdropFilter:"blur(4px)" }}>
                <div style={{ fontSize:"0.8rem",color:"#ffd60a",fontFamily:"sans-serif",fontWeight:"700",marginBottom:"0.4rem",letterSpacing:"0.05em",textTransform:"uppercase" }}>⚡ Quick Add from LEGO.com URL</div>
                <div style={{ fontSize:"0.72rem",color:"#666",fontFamily:"sans-serif",marginBottom:"0.75rem" }}>Paste any lego.com product URL and we'll look up all details automatically</div>
                <div style={{ display:"flex",gap:"0.5rem" }}>
                  <input value={urlInput} onChange={e=>setUrlInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&importFromUrl("Wanted")} placeholder="https://www.lego.com/en-au/product/..." style={{...IS,flex:1,background:"rgba(15,14,23,0.8)"}}/>
                  <button onClick={()=>importFromUrl("Wanted")} disabled={urlLoading} style={{ background:urlLoading?"#333":"#e63946",color:"#fff",border:"none",padding:"0.55rem 1.25rem",borderRadius:"6px",fontWeight:"900",cursor:urlLoading?"not-allowed":"pointer",fontFamily:"'Impact',sans-serif",fontSize:"0.9rem",whiteSpace:"nowrap" }}>{urlLoading?"...":"ADD ⭐"}</button>
                </div>
              </div>}
              {wishlist.length===0?(
                <div style={{ textAlign:"center",color:"#555",padding:"4rem",fontFamily:"sans-serif" }}><div style={{ fontSize:"3rem" }}>⭐</div><div style={{ marginTop:"0.5rem" }}>No sets on your wishlist yet</div></div>
              ):(
                <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(230px,1fr))",gap:"1rem" }}>
                  {wishlist.map(set=>(
                    <div key={set.id} style={{ background:"rgba(26,26,46,0.92)",border:"1px solid #ec489944",borderRadius:"12px",overflow:"hidden",backdropFilter:"blur(4px)" }}>
                      ...
                      <div style={{ padding:"0.85rem",display:"flex",flexDirection:"column",gap:"0.5rem" }}>
                        ...
                        {set.setNumber&&<a href={legoUrl(set.setNumber,set.name)} target="_blank" rel="noreferrer" style={{ background:"#ffd60a22",color:"#ffd60a",border:"1px solid #ffd60a44",padding:"0.4rem",borderRadius:"6px",fontSize:"0.75rem",fontFamily:"sans-serif",textDecoration:"none",textAlign:"center",display:"block" }}>🛒 View on LEGO.com</a>}
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {activeTab==="Minifigures"&&(
            <>
              <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"1.25rem",flexWrap:"wrap",gap:"0.75rem" }}>
                ...
                <input placeholder="🔍 Search minifigs or sets..." value={minifigSearch} onChange={e=>setMinifigSearch(e.target.value)} style={{...IS,width:"260px",background:"rgba(26,26,46,0.9)"}}/>
              </div>
              {filteredMinifigs.length===0?(
                <div style={{ textAlign:"center",color:"#555",padding:"4rem",fontFamily:"sans-serif" }}><div style={{ fontSize:"3rem" }}>🧑</div><div style={{ marginTop:"0.5rem" }}>No minifigures found</div></div>
              ):(
          ...
          {activeTab==="Stats"&&(()=>{
            ...
            const byTheme = {}; ownedSets.forEach(s=>{ byTheme[s.theme]=(byTheme[s.theme]||0)+1; });
            const themeData = Object.entries(byTheme).sort((a,b)=>b[1]-a[1]);
            const maxTheme = themeData[0]?.[1]||1;
            const byYear = {}; ownedSets.forEach(s=>{ if(s.year) byYear[s.year]=(byYear[s.year]||0)+1; });
            const yearData = Object.entries(byYear).sort((a,b)=>a[0]-b[0]);
            const maxYear = Math.max(...yearData.map(([,v])=>v),1);
            const byStatus = {}; sets.forEach(s=>{ byStatus[s.status]=(byStatus[s.status]||0)+1; });
            const byFolder = {}; ownedSets.forEach(s=>{ const f=s.folder||"No Folder"; byFolder[f]=(byFolder[f]||0)+1; });
            ...
            return (
              <>
                ...
                <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(150px,1fr))",gap:"0.75rem",marginBottom:"2rem" }}>
                  {[["Total Sets",ownedSets.length,"#3b82f6"],["Total Pieces",ownedSets.reduce((s,x)=>s+(parseInt(x.pieces)||0),0).toLocaleString(),"#22c55e"],["Total Minifigs",ownedSets.reduce((s,x)=>s+(parseInt(x.minifigs)||0),0).toLocaleString(),"#a855f7"],["Themes Covered",Object.keys(byTheme).length,"#f59e0b"],["Wishlist",sets.filter(s=>s.status==="Wanted").length,"#ec4899"],
                        ...
                        <div key={theme}>
                          <div style={{ display:"flex",justifyContent:"space-between",marginBottom:"0.2rem" }}>
                            <span style={{ fontSize:"0.75rem",fontFamily:"sans-serif",color:"#ccc",display:"flex",alignItems:"center",gap:"0.35rem" }}>
                              <span style={{ width:"8px",height:"8px",borderRadius:"50%",background:THEME_COLORS[theme]||"#666",display:"inline-block",flexShrink:0 }}/>{theme}
                            </span>
                          ...
                          <div style={{ background:"#0f0e17",borderRadius:"100px",height:"6px",overflow:"hidden" }}>
                            <div style={{ width:`${(count/maxTheme)*100}%`,height:"100%",background:THEME_COLORS[theme]||"#666",borderRadius:"100px" }}/>
                          </div>
                        </div>
                      ))}
                    </div>
                      ...
                      {Object.entries(byStatus).sort((a,b)=>b[1]-a[1]).map(([status,count])=>(
                        <div key={status} style={{ display:"flex",justifyContent:"space-between",alignItems:"center" }}>
                          <span style={{ fontSize:"0.78rem",fontFamily:"sans-serif",color:STATUS_COLORS[status]||"#aaa",display:"flex",alignItems:"center",gap:"0.4rem" }}>
                            <span style={{ width:"8px",height:"8px",borderRadius:"50%",background:STATUS_COLORS[status]||"#aaa",display:"inline-block" }}/>{status}
                          </span>
                    ...
                    <div style={{ display:"flex",flexDirection:"column",gap:"0.4rem",maxHeight:"180px",overflow:"auto" }}>
                      {folderData.map(([folder,count])=>(
                        <div key={folder} style={{ display:"flex",justifyContent:"space-between",fontSize:"0.75rem",fontFamily:"sans-serif",color:"#aaa" }}>
                          <span>📁 {folder}</span><span style={{ color:"#ffd60a",fontFamily:"monospace" }}>{count}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div style={{ display:"flex",alignItems:"flex-end",gap:"3px",height:"80px",overflowX:"auto",paddingBottom:"0.5rem" }}>
                    {yearData.map(([year,count])=>(
                      <div key={year} style={{ display:"flex",flexDirection:"column",alignItems:"center",gap:"0.2rem",minWidth:"28px" }}>
                        ...
                        <div style={{ width:"20px",background:"#e63946",borderRadius:"3px 3px 0 0",height:`${(count/maxYear)*60}px`,minHeight:"4px" }}/>
          ...
          {/* SET DETAIL MODAL */}
          {selectedSet&&(
            <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:100,padding:"1rem" }} onClick={()=>setSelectedSet(null)}>
              <div onClick={e=>e.stopPropagation()} style={{ background:"#1a1a2e",border:`2px solid ${THEME_COLORS[selectedSet.theme]||"#e63946"}`,borderRadius:"16px",maxWidth:"480px",width:"100%",maxHeight:"90vh",overflow:"auto" }}>
                <div style={{ height:"240px",background:"#fff",borderRadius:"14px 14px 0 0",overflow:"hidden" }}>
                  {(()=>{ const urls=selectedSet.imageUrl?[selectedSet.imageUrl]:getImageUrls(selectedSet.setNumber); return urls.length?<img src={urls[0]} alt="" style={{ width:"100%",height:"100%",objectFit:"contain" }} onError={e=>e.target.style.display="none"}/> : <div style={{ height:"100%",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"3rem" }}>🧱</div>; })()}
                </div>
                <div style={{ padding:"1.5rem" }}>
                  <div style={{ display:"inline-flex",alignItems:"center",gap:"0.4rem",background:(THEME_COLORS[selectedSet.theme]||"#666")+"22",color:THEME_COLORS[selectedSet.theme]||"#666",border:`1px solid ${THEME_COLORS[selectedSet.theme]||"#666"}44`,borderRadius:"100px",padding:"0.2rem 0.75rem",fontSize:"0.72rem",fontFamily:"sans-serif",fontWeight:"700",textTransform:"uppercase",letterSpacing:"0.05em",marginBottom:"0.75rem" }}>
                    <span style={{ width:"6px",height:"6px",borderRadius:"50%",background:THEME_COLORS[selectedSet.theme]||"#666" }}/>{selectedSet.theme}
                  </div>
                  ...
                  <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0.6rem",marginBottom:"1rem" }}>
                    {[["📅 Year",selectedSet.year],["🔢 Pieces",selectedSet.pieces?Number(selectedSet.pieces).toLocaleString():null],["🧑 Minifigs",selectedSet.minifigs&&selectedSet.minifigs!=="0"?selectedSet.minifigs:null],["📁 Folder",selectedSet.folder],["💰 Paid",selectedSet.purchasePrice?"$"+parseFloat(selectedSet.purchasePrice).toFixed(2):null],["🏷️ RRP",selectedSet.currentValue?"$"+parseFloat(selectedSet.currentValue).toFixed(2):null]].filter(([,v])=>v).map(([label,val])=>(
                  ...
                  {selectedSet.notes&&<div style={{ background:"rgba(255,255,255,0.03)",borderRadius:"8px",padding:"0.75rem",marginBottom:"1rem",fontSize:"0.78rem",color:"#888",fontFamily:"sans-serif",lineHeight:1.6 }}>{selectedSet.notes}</div>}
                  {selectedSet.purchasePrice&&selectedSet.currentValue&&(()=>{ const g=parseFloat(selectedSet.currentValue)-parseFloat(selectedSet.purchasePrice); return <div style={{ fontSize:"0.85rem",color:g>=0?"#22c55e":"#ef4444",fontFamily:"sans-serif",fontWeight:"700",marginBottom:"1rem" }}>{g>=0?"▲ Up":"▼ Down"} ${Math.abs(g).toFixed(2)} since purchase</div>; })()}
                  <div style={{ display:"flex",gap:"0.5rem",flexWrap:"wrap" }}>
                    {selectedSet.setNumber&&<a href={legoUrl(selectedSet.setNumber,selectedSet.name)} target="_blank" rel="noreferrer" style={{ flex:1,background:"#ffd60a22",color:"#ffd60a",border:"1px solid #ffd60a44",padding:"0.55rem",borderRadius:"8px",textDecoration:"none",textAlign:"center",fontSize:"0.8rem",fontFamily:"sans-serif",fontWeight:"700" }}>🛒 LEGO.com</a>}
          ...
          {showPasswordPrompt&&(
            <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:100 }}>
              <div style={{ background:"#1a1a2e",border:"2px solid #ffd60a",borderRadius:"12px",padding:"2rem",maxWidth:"320px",width:"90%",fontFamily:"sans-serif" }}>
                ...
                <input type="password" placeholder="Password" value={passwordInput} onChange={e=>{ setPasswordInput(e.target.value); setPasswordError(false); }} onKeyDown={e=>e.key==="Enter"&&attemptLogin()} autoFocus style={{...IS,marginBottom:"0.5rem",background:"#0f0e17"}}/>
          ...
          <div style={{ marginTop:"2rem",display:"flex",justifyContent:"center",gap:"1rem",flexWrap:"wrap" }}>
            <button onClick={()=>exportCSV(sets)} style={{ background:"rgba(26,26,46,0.9)",color:"#22c55e",border:"1px solid #22c55e44",padding:"0.6rem 1.25rem",borderRadius:"8px",cursor:"pointer",fontSize:"0.85rem",fontFamily:"sans-serif",fontWeight:"700" }}>📥 Export Full Collection (CSV)</button>
            <button onClick={()=>exportCSV(sets.filter(s=>s.status==="Wanted"),"lego-wishlist")} style={{ background:"rgba(26,26,46,0.9)",color:"#ec4899",border:"1px solid #ec489944",padding:"0.6rem 1.25rem",borderRadius:"8px",cursor:"pointer",fontSize:"0.85rem",fontFamily:"sans-serif",fontWeight:"700" }}>📥 Export Wishlist (CSV)</button>
          </div>
